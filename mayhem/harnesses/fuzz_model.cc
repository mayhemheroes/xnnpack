...
*/
//
// Ported verbatim from upstream's OSS-Fuzz recipe (google/oss-fuzz projects/xnnpack/fuzz_model.cc,
// as of the 2023 vintage still current for this project). Fuzzes xnn_create_convolution2d_nhwc_qs8
// (a quantized int8 2D convolution operator constructor) with fuzzer-controlled scales and weight
// bytes; kernel/channel/stride geometry is fixed so every input exercises the same code path with
// varying numeric content (a classic OSS-Fuzz "construct with adversarial numerics" harness).
